...
```bash
lake -KproofMode=native --reconfigure build CoveringCodes.Database.GeneratedAPI covering_codes
lake -KproofMode=kernel --reconfigure build CoveringCodes.Database.GeneratedAPI covering_codes
```
...
memory. `scripts/check-generated.sh` and `lake -KproofMode=native env lean --run Tools/TableGen/Main.lean`
...
Main.lean           interpreted query: looks up precomputed entry, prints certified bounds + trace
CoveringCodes/CLI.lean  compiled launcher that delegates to Main.lean
```

### Generated versus handwritten files

The Lean theorem library, relation layer, source wrappers, certificate types,
closure engine, interpreted query, and compiled CLI launcher are handwritten.
The precomputed table files are generated by:
...
`lake -KproofMode=native env lean --run Tools/TableGen/Main.lean`, and use
...
6. **CLI display** (`Main.lean` and `CoveringCodes/CLI.lean`): `Main.lean`
   looks up a `CertifiedBoundEntry`, replays its traces into proofs, and prints
   the numbered derivation chain; the compiled launcher delegates to `Main.lean`
   so proof modules do not have to be exported to native C.
