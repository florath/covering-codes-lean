...
/-!
# Explicit upper bound for `K_14(6,4)`, verified via SAT-UNSAT + LRAT

`K_14(6,4) <= 50` is reformulated as: no assignment of a symbol to each of
the 6 coordinates makes the assignment simultaneously "far" (Hamming
distance > 4) from every one of the 50 codewords. This is exactly UNSAT of
...
The 50-word code is due to Mark Marosi, "New upper and lower bounds on
...
namespace Quattuordenary664LRAT
...
lrat_cnf_proof coverFormula coverProof
  (include_str "../../../../data/K_14_6_4/cover.cnf")
  (include_str "../../../../data/K_14_6_4/cover.lrat")

open Mathlib.Tactic.Sat (buildConj buildClause)

/-- 0-indexed SAT variable for "position `j` has symbol `v`". -/
def satVar (j v : Nat) : Nat := j * 14 + v

def atLeastClause (j : Nat) : Sat.Clause :=
  (List.range 14).map (fun v => Sat.Literal.pos (satVar j v))

def atMostClauses (j : Nat) : Sat.Fmla :=
  (List.range 14).flatMap (fun v1 =>
    ((List.range 14).filter (v1 < ·)).map (fun v2 =>
...
theorem coverFormula_eq_direct :
  ...
  have h1 : coverFormula.take 552 = oneHotClauses := by rfl
  have h2 : coverFormula.drop 552 = codeClauses code := by rfl
  have h := List.take_append_drop 552 coverFormula
  rw [h1, h2] at h
  exact h.symm

/-- The natural `Sat.Valuation` for a q-ary word: index `idx` is "on" iff
position `idx / 14` of `x` carries symbol `idx % 14`. -/
def qaryWordValuation (x : QaryWord 14 6) : Sat.Valuation :=
  fun idx => (x ⟨idx / 14 % 6, Nat.mod_lt _ (by decide)⟩).val = idx % 14

theorem qaryWordValuation_satVar (x : QaryWord 14 6) (j v : Nat) (hj : j < 6) (hv : v < 14) :
    qaryWordValuation x (satVar j v) ↔ (x ⟨j, hj⟩).val = v := by
  have hj' : (j * 14 + v) / 14 % 6 = j := by omega
  have hv' : (j * 14 + v) % 14 = v := by omega
  have hFin : (⟨(j * 14 + v) / 14 % 6, Nat.mod_lt _ (by decide)⟩ : Fin 6) = ⟨j, hj⟩ :=
    Fin.ext hj'
  show (x ⟨(j * 14 + v) / 14 % 6, Nat.mod_lt _ (by decide)⟩).val = (j * 14 + v) % 14 ↔
      (x ⟨j, hj⟩).val = v
  rw [hFin, hv']

theorem satisfies_atLeastClause (x : QaryWord 14 6) (j : Nat) (hj : j < 6) :
    (qaryWordValuation x).satisfies (atLeastClause j) := by
  have hlt : (x ⟨j, hj⟩).val < 14 := (x ⟨j, hj⟩).isLt
...
theorem satisfies_atMostPair (x : QaryWord 14 6) (j v1 v2 : Nat) (hj : j < 6) (hv1 : v1 < 14)
    (hv2 : v2 < 14) (hne : v1 ≠ v2) :
...
theorem satisfies_atMostClauses (x : QaryWord 14 6) (j : Nat) (hj : j < 6) (clause : Sat.Clause)
  ...
  have hv1 : v1 < 14 := List.mem_range.mp hv1r
  have hv2 : v2 < 14 := List.mem_range.mp hv2r
  have hlt' : v1 < v2 := by simpa using hlt
  exact satisfies_atMostPair x j v1 v2 hj hv1 hv2 hlt'.ne

theorem satisfies_oneHotClauses (x : QaryWord 14 6) (clause : Sat.Clause)
...
def xVal (x : QaryWord 14 6) (j : Nat) : Nat :=
...
theorem satisfies_farClauseOne (x : QaryWord 14 6) (c : List Nat) (T : List Nat)
    (hTN : ∀ j ∈ T, j < 6) (hcQ : ∀ j, c.getD j 0 < 14)
...
theorem satisfies_farClauses (x : QaryWord 14 6) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 14)
...
theorem satisfies_codeClauses (x : QaryWord 14 6) (code : List (List Nat))
    (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 14)
...
/-- The actual covering theorem: every `QaryWord 14 6` is within Hamming
...
theorem covers (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 14) (x : QaryWord 14 6) :
...
/-- Convert a length-6, digits-`<14` `List Nat` codeword into an actual
`QaryWord 14 6`. The `% 14` is a safety net (irrelevant for well-formed
codewords, where `c.getD j 0 < 14` already). -/
def toQaryWord (c : List Nat) : QaryWord 14 6 :=
  fun i => ⟨c.getD i.val 0 % 14, Nat.mod_lt _ (by decide)⟩

theorem hammingDist_le_of_count_le (x : QaryWord 14 6) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 14)
  ...
  have hcard : (Finset.univ.filter (fun i : Fin 6 => x i ≠ toQaryWord c i)).card ≤ n := by
    ...
    have hmaps : ∀ i ∈ Finset.univ.filter (fun i : Fin 6 => x i ≠ toQaryWord c i),
      ...
      have hyi : (toQaryWord c) i = ⟨c.getD i.val 0 % 14, Nat.mod_lt _ (by decide)⟩ := rfl
      have hmod : c.getD i.val 0 % 14 = c.getD i.val 0 := Nat.mod_eq_of_lt (hcQ i.val)
      apply Fin.ext
      rw [hyi]
      simp only [hmod]
      rw [← hxi, heq]
...
theorem getD_lt_of_forall_mem_lt {c : List Nat} (h : ∀ d ∈ c, d < 14) : ∀ j, c.getD j 0 < 14 := by
  induction c with
...
theorem code_entries_ltQ : ∀ c ∈ code, ∀ d ∈ c, d < 14 := by decide

theorem code_getD_lt : ∀ c ∈ code, ∀ j, c.getD j 0 < 14 :=
  fun c hc => getD_lt_of_forall_mem_lt (code_entries_ltQ c hc)

def codeFinset : Finset (QaryWord 14 6) := (code.map toQaryWord).toFinset

set_option maxRecDepth 10000 in
theorem code_length : code.length = 50 := by decide

theorem codeFinset_card_le : codeFinset.card ≤ 50 := by
...
def explicitUpper : ExplicitQaryUpper 14 6 4 50 :=
  { code := codeFinset
...
def upper (q n r : Nat) : Nat :=
  if q = 14 ∧ n = 6 ∧ r = 4 then 50 else trivialUpper q n r

theorem upper_valid (q n r : Nat) :
    QaryKUpper q n r (upper q n r) := by
  by_cases h : q = 14 ∧ n = 6 ∧ r = 4
