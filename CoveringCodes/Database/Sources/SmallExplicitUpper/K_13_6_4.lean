...
/-!
# Explicit upper bound for `K_13(6,4)`, verified via SAT-UNSAT + LRAT

`K_13(6,4) <= 45` is reformulated as: no assignment of a symbol to each of
the 6 coordinates makes the assignment simultaneously "far" (Hamming
distance > 4) from every one of the 45 codewords. This is exactly UNSAT of
...
The 45-word code is due to Mark Marosi, "New upper and lower bounds on
...
lrat_cnf_proof coverFormula coverProof
  (include_str "../../../../data/K_13_6_4/cover.cnf")
  (include_str "../../../../data/K_13_6_4/cover.lrat")

open Mathlib.Tactic.Sat (buildConj buildClause)

/-- 0-indexed SAT variable for "position `j` has symbol `v`". -/
def satVar (j v : Nat) : Nat := j * 13 + v

def atLeastClause (j : Nat) : Sat.Clause :=
  (List.range 13).map (fun v => Sat.Literal.pos (satVar j v))

def atMostClauses (j : Nat) : Sat.Fmla :=
  (List.range 13).flatMap (fun v1 =>
    ((List.range 13).filter (v1 < ·)).map (fun v2 =>
...
theorem coverFormula_eq_direct :
  ...
  have h1 : coverFormula.take 474 = oneHotClauses := by rfl
  have h2 : coverFormula.drop 474 = codeClauses code := by rfl
  have h := List.take_append_drop 474 coverFormula
  rw [h1, h2] at h
  exact h.symm

/-- The natural `Sat.Valuation` for a q-ary word: index `idx` is "on" iff
position `idx / 13` of `x` carries symbol `idx % 13`. -/
def qaryWordValuation (x : QaryWord 13 6) : Sat.Valuation :=
  fun idx => (x ⟨idx / 13 % 6, Nat.mod_lt _ (by decide)⟩).val = idx % 13

theorem qaryWordValuation_satVar (x : QaryWord 13 6) (j v : Nat) (hj : j < 6) (hv : v < 13) :
    qaryWordValuation x (satVar j v) ↔ (x ⟨j, hj⟩).val = v := by
  have hj' : (j * 13 + v) / 13 % 6 = j := by omega
  have hv' : (j * 13 + v) % 13 = v := by omega
  have hFin : (⟨(j * 13 + v) / 13 % 6, Nat.mod_lt _ (by decide)⟩ : Fin 6) = ⟨j, hj⟩ :=
    Fin.ext hj'
  show (x ⟨(j * 13 + v) / 13 % 6, Nat.mod_lt _ (by decide)⟩).val = (j * 13 + v) % 13 ↔
      (x ⟨j, hj⟩).val = v
  rw [hFin, hv']

theorem satisfies_atLeastClause (x : QaryWord 13 6) (j : Nat) (hj : j < 6) :
    (qaryWordValuation x).satisfies (atLeastClause j) := by
  have hlt : (x ⟨j, hj⟩).val < 13 := (x ⟨j, hj⟩).isLt
...
theorem satisfies_atMostPair (x : QaryWord 13 6) (j v1 v2 : Nat) (hj : j < 6) (hv1 : v1 < 13)
    (hv2 : v2 < 13) (hne : v1 ≠ v2) :
...
theorem satisfies_atMostClauses (x : QaryWord 13 6) (j : Nat) (hj : j < 6) (clause : Sat.Clause)
  ...
  have hv1 : v1 < 13 := List.mem_range.mp hv1r
  have hv2 : v2 < 13 := List.mem_range.mp hv2r
  have hlt' : v1 < v2 := by simpa using hlt
  exact satisfies_atMostPair x j v1 v2 hj hv1 hv2 hlt'.ne

theorem satisfies_oneHotClauses (x : QaryWord 13 6) (clause : Sat.Clause)
...
def xVal (x : QaryWord 13 6) (j : Nat) : Nat :=
...
theorem satisfies_farClauseOne (x : QaryWord 13 6) (c : List Nat) (T : List Nat)
    (hTN : ∀ j ∈ T, j < 6) (hcQ : ∀ j, c.getD j 0 < 13)
...
theorem satisfies_farClauses (x : QaryWord 13 6) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 13)
...
theorem satisfies_codeClauses (x : QaryWord 13 6) (code : List (List Nat))
    (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 13)
...
/-- The actual covering theorem: every `QaryWord 13 6` is within Hamming
...
theorem covers (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 13) (x : QaryWord 13 6) :
...
/-- Convert a length-6, digits-`<13` `List Nat` codeword into an actual
`QaryWord 13 6`. The `% 13` is a safety net (irrelevant for well-formed
codewords, where `c.getD j 0 < 13` already). -/
def toQaryWord (c : List Nat) : QaryWord 13 6 :=
  fun i => ⟨c.getD i.val 0 % 13, Nat.mod_lt _ (by decide)⟩

theorem hammingDist_le_of_count_le (x : QaryWord 13 6) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 13)
  ...
  have hcard : (Finset.univ.filter (fun i : Fin 6 => x i ≠ toQaryWord c i)).card ≤ n := by
    ...
    have hmaps : ∀ i ∈ Finset.univ.filter (fun i : Fin 6 => x i ≠ toQaryWord c i),
      ...
      have hyi : (toQaryWord c) i = ⟨c.getD i.val 0 % 13, Nat.mod_lt _ (by decide)⟩ := rfl
      have hmod : c.getD i.val 0 % 13 = c.getD i.val 0 := Nat.mod_eq_of_lt (hcQ i.val)
      apply Fin.ext
      rw [hyi]
      simp only [hmod]
      rw [← hxi, heq]
...
theorem getD_lt_of_forall_mem_lt {c : List Nat} (h : ∀ d ∈ c, d < 13) : ∀ j, c.getD j 0 < 13 := by
  induction c with
...
theorem code_entries_ltQ : ∀ c ∈ code, ∀ d ∈ c, d < 13 := by decide

theorem code_getD_lt : ∀ c ∈ code, ∀ j, c.getD j 0 < 13 :=
  fun c hc => getD_lt_of_forall_mem_lt (code_entries_ltQ c hc)

def codeFinset : Finset (QaryWord 13 6) := (code.map toQaryWord).toFinset

set_option maxRecDepth 10000 in
theorem code_length : code.length = 45 := by decide

theorem codeFinset_card_le : codeFinset.card ≤ 45 := by
...
def explicitUpper : ExplicitQaryUpper 13 6 4 45 :=
  { code := codeFinset
...
def upper (q n r : Nat) : Nat :=
  if q = 13 ∧ n = 6 ∧ r = 4 then 45 else trivialUpper q n r

theorem upper_valid (q n r : Nat) :
    QaryKUpper q n r (upper q n r) := by
  by_cases h : q = 13 ∧ n = 6 ∧ r = 4
