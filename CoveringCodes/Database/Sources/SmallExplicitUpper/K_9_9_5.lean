...
/-!
# Explicit upper bound for `K_9(9,5)`, verified via SAT-UNSAT + LRAT

`K_9(9,5) <= 684` is reformulated as: no assignment of a symbol to each of
the 9 coordinates makes the assignment simultaneously "far" (Hamming
distance > 5) from every one of the 684 codewords. This is exactly UNSAT of
...
The 684-word code is due to Mark Marosi, "New upper and lower bounds on
...
lrat_cnf_proof coverFormula coverProof
  (include_str "../../../../data/K_9_9_5/cover.cnf")
  (include_str "../../../../data/K_9_9_5/cover.lrat")

open Mathlib.Tactic.Sat (buildConj buildClause)

/-- 0-indexed SAT variable for "position `j` has symbol `v`". -/
def satVar (j v : Nat) : Nat := j * 9 + v

def atLeastClause (j : Nat) : Sat.Clause :=
  (List.range 9).map (fun v => Sat.Literal.pos (satVar j v))

def atMostClauses (j : Nat) : Sat.Fmla :=
  (List.range 9).flatMap (fun v1 =>
    ((List.range 9).filter (v1 < ·)).map (fun v2 =>
...
theorem coverFormula_eq_direct :
  ...
  have h1 : coverFormula.take 333 = oneHotClauses := by rfl
  have h2 : coverFormula.drop 333 = codeClauses code := by rfl
  have h := List.take_append_drop 333 coverFormula
  rw [h1, h2] at h
  exact h.symm

/-- The natural `Sat.Valuation` for a q-ary word: index `idx` is "on" iff
position `idx / 9` of `x` carries symbol `idx % 9`. -/
def qaryWordValuation (x : QaryWord 9 9) : Sat.Valuation :=
  fun idx => (x ⟨idx / 9 % 9, Nat.mod_lt _ (by decide)⟩).val = idx % 9

theorem qaryWordValuation_satVar (x : QaryWord 9 9) (j v : Nat) (hj : j < 9) (hv : v < 9) :
    qaryWordValuation x (satVar j v) ↔ (x ⟨j, hj⟩).val = v := by
  have hj' : (j * 9 + v) / 9 % 9 = j := by omega
  have hv' : (j * 9 + v) % 9 = v := by omega
  have hFin : (⟨(j * 9 + v) / 9 % 9, Nat.mod_lt _ (by decide)⟩ : Fin 9) = ⟨j, hj⟩ :=
    Fin.ext hj'
  show (x ⟨(j * 9 + v) / 9 % 9, Nat.mod_lt _ (by decide)⟩).val = (j * 9 + v) % 9 ↔
      (x ⟨j, hj⟩).val = v
  rw [hFin, hv']

theorem satisfies_atLeastClause (x : QaryWord 9 9) (j : Nat) (hj : j < 9) :
    (qaryWordValuation x).satisfies (atLeastClause j) := by
  have hlt : (x ⟨j, hj⟩).val < 9 := (x ⟨j, hj⟩).isLt
...
theorem satisfies_atMostPair (x : QaryWord 9 9) (j v1 v2 : Nat) (hj : j < 9) (hv1 : v1 < 9)
    (hv2 : v2 < 9) (hne : v1 ≠ v2) :
...
theorem satisfies_atMostClauses (x : QaryWord 9 9) (j : Nat) (hj : j < 9) (clause : Sat.Clause)
  ...
  have hv1 : v1 < 9 := List.mem_range.mp hv1r
  have hv2 : v2 < 9 := List.mem_range.mp hv2r
  have hlt' : v1 < v2 := by simpa using hlt
  exact satisfies_atMostPair x j v1 v2 hj hv1 hv2 hlt'.ne

theorem satisfies_oneHotClauses (x : QaryWord 9 9) (clause : Sat.Clause)
...
def xVal (x : QaryWord 9 9) (j : Nat) : Nat :=
...
theorem satisfies_farClauseOne (x : QaryWord 9 9) (c : List Nat) (T : List Nat)
    (hTN : ∀ j ∈ T, j < 9) (hcQ : ∀ j, c.getD j 0 < 9)
...
theorem satisfies_farClauses (x : QaryWord 9 9) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 9)
...
theorem satisfies_codeClauses (x : QaryWord 9 9) (code : List (List Nat))
    (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 9)
...
/-- The actual covering theorem: every `QaryWord 9 9` is within Hamming
...
theorem covers (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 9) (x : QaryWord 9 9) :
...
/-- Convert a length-9, digits-`<9` `List Nat` codeword into an actual
`QaryWord 9 9`. The `% 9` is a safety net (irrelevant for well-formed
codewords, where `c.getD j 0 < 9` already). -/
def toQaryWord (c : List Nat) : QaryWord 9 9 :=
  fun i => ⟨c.getD i.val 0 % 9, Nat.mod_lt _ (by decide)⟩

theorem hammingDist_le_of_count_le (x : QaryWord 9 9) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 9)
  ...
  have hcard : (Finset.univ.filter (fun i : Fin 9 => x i ≠ toQaryWord c i)).card ≤ n := by
    ...
    have hmaps : ∀ i ∈ Finset.univ.filter (fun i : Fin 9 => x i ≠ toQaryWord c i),
      ...
      have hyi : (toQaryWord c) i = ⟨c.getD i.val 0 % 9, Nat.mod_lt _ (by decide)⟩ := rfl
      have hmod : c.getD i.val 0 % 9 = c.getD i.val 0 := Nat.mod_eq_of_lt (hcQ i.val)
      apply Fin.ext
      rw [hyi]
      simp only [hmod]
      rw [← hxi, heq]
...
theorem getD_lt_of_forall_mem_lt {c : List Nat} (h : ∀ d ∈ c, d < 9) : ∀ j, c.getD j 0 < 9 := by
  induction c with
...
theorem code_entries_ltQ : ∀ c ∈ code, ∀ d ∈ c, d < 9 := by decide

theorem code_getD_lt : ∀ c ∈ code, ∀ j, c.getD j 0 < 9 :=
  fun c hc => getD_lt_of_forall_mem_lt (code_entries_ltQ c hc)

def codeFinset : Finset (QaryWord 9 9) := (code.map toQaryWord).toFinset

set_option maxRecDepth 10000 in
theorem code_length : code.length = 684 := by decide

theorem codeFinset_card_le : codeFinset.card ≤ 684 := by
...
def explicitUpper : ExplicitQaryUpper 9 9 5 684 :=
  { code := codeFinset
...
def upper (q n r : Nat) : Nat :=
  if q = 9 ∧ n = 9 ∧ r = 5 then 684 else trivialUpper q n r
