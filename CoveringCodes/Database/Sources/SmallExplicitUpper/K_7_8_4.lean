...
/-!
# Explicit upper bound for `K_7(8,4)`, verified via SAT-UNSAT + LRAT

`K_7(8,4) <= 316` is reformulated as: no assignment of a symbol to each of
the 8 coordinates makes the assignment simultaneously "far" (Hamming
distance > 4) from every one of the 316 codewords. This is exactly UNSAT of
...
The 316-word code is due to Mark Marosi, "New upper and lower bounds on
...
lrat_cnf_proof coverFormula coverProof
  (include_str "../../../../data/K_7_8_4/cover.cnf")
  (include_str "../../../../data/K_7_8_4/cover.lrat")
...
theorem coverFormula_eq_direct :
  ...
  have h1 : coverFormula.take 176 = oneHotClauses := by rfl
  have h2 : coverFormula.drop 176 = codeClauses code := by rfl
  have h := List.take_append_drop 176 coverFormula
  rw [h1, h2] at h
  exact h.symm

/-- The natural `Sat.Valuation` for a q-ary word: index `idx` is "on" iff
position `idx / 7` of `x` carries symbol `idx % 7`. -/
def qaryWordValuation (x : QaryWord 7 8) : Sat.Valuation :=
  fun idx => (x ⟨idx / 7 % 8, Nat.mod_lt _ (by decide)⟩).val = idx % 7

theorem qaryWordValuation_satVar (x : QaryWord 7 8) (j v : Nat) (hj : j < 8) (hv : v < 7) :
    qaryWordValuation x (satVar j v) ↔ (x ⟨j, hj⟩).val = v := by
  have hj' : (j * 7 + v) / 7 % 8 = j := by omega
  have hv' : (j * 7 + v) % 7 = v := by omega
  have hFin : (⟨(j * 7 + v) / 7 % 8, Nat.mod_lt _ (by decide)⟩ : Fin 8) = ⟨j, hj⟩ :=
    Fin.ext hj'
  show (x ⟨(j * 7 + v) / 7 % 8, Nat.mod_lt _ (by decide)⟩).val = (j * 7 + v) % 7 ↔
      (x ⟨j, hj⟩).val = v
  rw [hFin, hv']

theorem satisfies_atLeastClause (x : QaryWord 7 8) (j : Nat) (hj : j < 8) :
...
theorem satisfies_atMostPair (x : QaryWord 7 8) (j v1 v2 : Nat) (hj : j < 8) (hv1 : v1 < 7)
    (hv2 : v2 < 7) (hne : v1 ≠ v2) :
...
theorem satisfies_atMostClauses (x : QaryWord 7 8) (j : Nat) (hj : j < 8) (clause : Sat.Clause)
...
theorem satisfies_oneHotClauses (x : QaryWord 7 8) (clause : Sat.Clause)
...
def xVal (x : QaryWord 7 8) (j : Nat) : Nat :=
...
theorem satisfies_farClauseOne (x : QaryWord 7 8) (c : List Nat) (T : List Nat)
    (hTN : ∀ j ∈ T, j < 8) (hcQ : ∀ j, c.getD j 0 < 7)
...
theorem satisfies_farClauses (x : QaryWord 7 8) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 7)
...
theorem satisfies_codeClauses (x : QaryWord 7 8) (code : List (List Nat))
...
/-- The actual covering theorem: every `QaryWord 7 8` is within Hamming
...
theorem covers (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 7) (x : QaryWord 7 8) :
...
/-- Convert a length-8, digits-`<7` `List Nat` codeword into an actual
`QaryWord 7 8`. The `% 7` is a safety net (irrelevant for well-formed
codewords, where `c.getD j 0 < 7` already). -/
def toQaryWord (c : List Nat) : QaryWord 7 8 :=
  fun i => ⟨c.getD i.val 0 % 7, Nat.mod_lt _ (by decide)⟩

theorem hammingDist_le_of_count_le (x : QaryWord 7 8) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 7)
...
def codeFinset : Finset (QaryWord 7 8) := (code.map toQaryWord).toFinset

set_option maxRecDepth 10000 in
theorem code_length : code.length = 316 := by decide

theorem codeFinset_card_le : codeFinset.card ≤ 316 := by
...
def explicitUpper : ExplicitQaryUpper 7 8 4 316 :=
  { code := codeFinset
...
def upper (q n r : Nat) : Nat :=
  if q = 7 ∧ n = 8 ∧ r = 4 then 316 else trivialUpper q n r
