...
/-!
# Explicit upper bound for `K_15(6,4)`, verified via SAT-UNSAT + LRAT

`K_15(6,4) <= 57` is reformulated as: no assignment of a symbol to each of
the 6 coordinates makes the assignment simultaneously "far" (Hamming
distance > 4) from every one of the 57 codewords. This is exactly UNSAT of
a CNF built from one-hot coordinate-symbol variables, plus, per codeword, a
"disagree in >= 5 positions" cardinality constraint (encoded directly as
clauses over the negated one-hot literals -- no auxiliary variables).

The 57-word code is due to Mark Marosi, "New upper and lower bounds on
...
lrat_cnf_proof coverFormula coverProof
  (include_str "../../../../data/K_15_6_4/cover.cnf")
  (include_str "../../../../data/K_15_6_4/cover.lrat")

open Mathlib.Tactic.Sat (buildConj buildClause)

/-- 0-indexed SAT variable for "position `j` has symbol `v`". -/
def satVar (j v : Nat) : Nat := j * 15 + v

def atLeastClause (j : Nat) : Sat.Clause :=
  (List.range 15).map (fun v => Sat.Literal.pos (satVar j v))

def atMostClauses (j : Nat) : Sat.Fmla :=
  (List.range 15).flatMap (fun v1 =>
    ((List.range 15).filter (v1 < ·)).map (fun v2 =>
      ([Sat.Literal.neg (satVar j v1), Sat.Literal.neg (satVar j v2)] : Sat.Clause)))

def oneHotClauses : Sat.Fmla :=
  (List.range 6).flatMap (fun j => atLeastClause j :: atMostClauses j)
...
/-- The 15 (= C(6,2)) "far from `c`" clauses: for every size-2 subset of
positions, at least one of them must disagree with `c`. -/
def farClauses (c : List Nat) : Sat.Fmla :=
  (combinations (List.range 6) 2).map (fun T =>
...
theorem coverFormula_eq_direct :
  ...
  have h1 : coverFormula.take 636 = oneHotClauses := by rfl
  have h2 : coverFormula.drop 636 = codeClauses code := by rfl
  have h := List.take_append_drop 636 coverFormula
  rw [h1, h2] at h
  exact h.symm

/-- The natural `Sat.Valuation` for a q-ary word: index `idx` is "on" iff
position `idx / 15` of `x` carries symbol `idx % 15`. -/
def qaryWordValuation (x : QaryWord 15 6) : Sat.Valuation :=
  fun idx => (x ⟨idx / 15 % 6, Nat.mod_lt _ (by decide)⟩).val = idx % 15

theorem qaryWordValuation_satVar (x : QaryWord 15 6) (j v : Nat) (hj : j < 6) (hv : v < 15) :
    qaryWordValuation x (satVar j v) ↔ (x ⟨j, hj⟩).val = v := by
  have hj' : (j * 15 + v) / 15 % 6 = j := by omega
  have hv' : (j * 15 + v) % 15 = v := by omega
  have hFin : (⟨(j * 15 + v) / 15 % 6, Nat.mod_lt _ (by decide)⟩ : Fin 6) = ⟨j, hj⟩ :=
    Fin.ext hj'
  show (x ⟨(j * 15 + v) / 15 % 6, Nat.mod_lt _ (by decide)⟩).val = (j * 15 + v) % 15 ↔
      (x ⟨j, hj⟩).val = v
  rw [hFin, hv']

theorem satisfies_atLeastClause (x : QaryWord 15 6) (j : Nat) (hj : j < 6) :
    (qaryWordValuation x).satisfies (atLeastClause j) := by
  have hlt : (x ⟨j, hj⟩).val < 15 := (x ⟨j, hj⟩).isLt
  show (qaryWordValuation x).neg (Sat.Literal.pos (satVar j 0)) →
       ...
       (qaryWordValuation x).neg (Sat.Literal.pos (satVar j 7)) →
       (qaryWordValuation x).neg (Sat.Literal.pos (satVar j 8)) →
       (qaryWordValuation x).neg (Sat.Literal.pos (satVar j 9)) →
       (qaryWordValuation x).neg (Sat.Literal.pos (satVar j 10)) →
       (qaryWordValuation x).neg (Sat.Literal.pos (satVar j 11)) →
       (qaryWordValuation x).neg (Sat.Literal.pos (satVar j 12)) →
       (qaryWordValuation x).neg (Sat.Literal.pos (satVar j 13)) →
       (qaryWordValuation x).neg (Sat.Literal.pos (satVar j 14)) →
       False
  simp only [Sat.Valuation.neg, qaryWordValuation_satVar x j 0 hj (by omega),
    ...
    qaryWordValuation_satVar x j 7 hj (by omega),
    qaryWordValuation_satVar x j 8 hj (by omega),
    qaryWordValuation_satVar x j 9 hj (by omega),
    qaryWordValuation_satVar x j 10 hj (by omega),
    qaryWordValuation_satVar x j 11 hj (by omega),
    qaryWordValuation_satVar x j 12 hj (by omega),
    qaryWordValuation_satVar x j 13 hj (by omega),
    qaryWordValuation_satVar x j 14 hj (by omega)]
  omega

theorem satisfies_atMostPair (x : QaryWord 15 6) (j v1 v2 : Nat) (hj : j < 6) (hv1 : v1 < 15)
    (hv2 : v2 < 15) (hne : v1 ≠ v2) :
...
theorem satisfies_atMostClauses (x : QaryWord 15 6) (j : Nat) (hj : j < 6) (clause : Sat.Clause)
...
/-- Pigeonhole: if `f` and `g` disagree on at least 5 of the 6 positions,
every size-2 subset of positions contains a disagreement. -/
...
theorem exists_disagree_of_far (f g : Nat → Nat)
    (hfar : 5 ≤ ((List.range 6).filter (fun j => decide (f j ≠ g j))).length) :
    ∀ T ∈ combinations (List.range 6) 2, ∃ j ∈ T, f j ≠ g j := by
  intro T hT
  by_contra hno
  push Not at hno
  obtain ⟨hsub, hlen⟩ := combinations_sublist (List.range 6) 2 T hT
  ...
  have hTfilter : T.filter (fun j => !p j) = T := by
    ...
    exact hno j hj
  have hTsub' : List.Sublist (T.filter (fun j => !p j)) ((List.range 6).filter (fun j => !p j)) :=
    List.Sublist.filter _ hsub
  rw [hTfilter] at hTsub'
  have hlen' : T.length ≤ ((List.range 6).filter (fun j => !p j)).length := hTsub'.length_le
  have hpart := length_filter_add_filter_not p (List.range 6)
  simp only [List.length_range] at hpart
  omega

/-- Total Nat-valued accessor for a `QaryWord`'s symbol at position `j`
(`0` out of range) -- avoids threading `j < 6` proofs through existentials. -/
def xVal (x : QaryWord 15 6) (j : Nat) : Nat :=
  if h : j < 6 then (x ⟨j, h⟩).val else 0
...
theorem satisfies_farClauseOne (x : QaryWord 15 6) (c : List Nat) (T : List Nat)
    (hTN : ∀ j ∈ T, j < 6) (hcQ : ∀ j, c.getD j 0 < 15)
...
theorem satisfies_farClauses (x : QaryWord 15 6) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 15)
    (hfar : 5 ≤ ((List.range 6).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length)
    (clause : Sat.Clause) (hmem : clause ∈ farClauses c) :
    (qaryWordValuation x).satisfies clause := by
  unfold farClauses at hmem
  obtain ⟨T, hT, rfl⟩ := List.mem_map.mp hmem
  have hTN : ∀ j ∈ T, j < 6 := fun j hj =>
...
theorem satisfies_codeClauses (x : QaryWord 15 6) (code : List (List Nat))
    (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 15)
    (hfar : ∀ c ∈ code, 5 ≤ ((List.range 6).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length)
...
/-- The actual covering theorem: every `QaryWord 15 6` is within Hamming
distance `4` of some codeword in `code` -- reached entirely via the
SAT-UNSAT/LRAT proof, no `decide`/`native_decide` anywhere in this chain. -/
theorem covers (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 15) (x : QaryWord 15 6) :
    ∃ c ∈ code, ((List.range 6).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length ≤ 4 := by
  by_contra hno
  push Not at hno
...
/-- Convert a length-6, digits-`<15` `List Nat` codeword into an actual
`QaryWord 15 6`. The `% 15` is a safety net (irrelevant for well-formed
codewords, where `c.getD j 0 < 15` already). -/
def toQaryWord (c : List Nat) : QaryWord 15 6 :=
  fun i => ⟨c.getD i.val 0 % 15, Nat.mod_lt _ (by decide)⟩

theorem hammingDist_le_of_count_le (x : QaryWord 15 6) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 15)
    (n : Nat) (hcount : ((List.range 6).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length ≤ n) :
    hammingDist x (toQaryWord c) ≤ n := by
  have hcard : (Finset.univ.filter (fun i : Fin 6 => x i ≠ toQaryWord c i)).card ≤ n := by
    have hinj : Set.InjOn Fin.val (Finset.univ.filter (fun i : Fin 6 => x i ≠ toQaryWord c i) : Set (Fin 6)) :=
      fun a _ b _ h => Fin.ext h
    have hmaps : ∀ i ∈ Finset.univ.filter (fun i : Fin 6 => x i ≠ toQaryWord c i),
        i.val ∈ ((List.range 6).filter (fun j => decide (xVal x j ≠ c.getD j 0))).toFinset := by
      intro i hi
      ...
      have hyi : (toQaryWord c) i = ⟨c.getD i.val 0 % 15, Nat.mod_lt _ (by decide)⟩ := rfl
      have hmod : c.getD i.val 0 % 15 = c.getD i.val 0 := Nat.mod_eq_of_lt (hcQ i.val)
      apply Fin.ext
      rw [hyi]
      simp only [hmod]
      rw [← hxi, heq]
    calc (Finset.univ.filter (fun i : Fin 6 => x i ≠ toQaryWord c i)).card
        ≤ ((List.range 6).filter (fun j => decide (xVal x j ≠ c.getD j 0))).toFinset.card :=
          Finset.card_le_card_of_injOn Fin.val hmaps hinj
      _ ≤ ((List.range 6).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length :=
          List.toFinset_card_le _
      _ ≤ n := hcount
  exact hcard

theorem getD_lt_of_forall_mem_lt {c : List Nat} (h : ∀ d ∈ c, d < 15) : ∀ j, c.getD j 0 < 15 := by
  induction c with
...
theorem code_entries_ltQ : ∀ c ∈ code, ∀ d ∈ c, d < 15 := by decide

theorem code_getD_lt : ∀ c ∈ code, ∀ j, c.getD j 0 < 15 :=
  fun c hc => getD_lt_of_forall_mem_lt (code_entries_ltQ c hc)

def codeFinset : Finset (QaryWord 15 6) := (code.map toQaryWord).toFinset

set_option maxRecDepth 10000 in
theorem code_length : code.length = 57 := by decide

theorem codeFinset_card_le : codeFinset.card ≤ 57 := by
  calc codeFinset.card
      ≤ (code.map toQaryWord).length := List.toFinset_card_le _
    _ = code.length := by rw [List.length_map]
    _ = 57 := code_length

theorem codeFinset_covers : CoversFinset codeFinset 4 := by
  intro x
  ...
  · exact hammingDist_le_of_count_le x c (code_getD_lt c hc) 4 hcount

def explicitUpper : ExplicitQaryUpper 15 6 4 57 :=
  { code := codeFinset
...
def upper (q n r : Nat) : Nat :=
  if q = 15 ∧ n = 6 ∧ r = 4 then 57 else trivialUpper q n r

theorem upper_valid (q n r : Nat) :
    QaryKUpper q n r (upper q n r) := by
  by_cases h : q = 15 ∧ n = 6 ∧ r = 4
