...
/-!
# Explicit upper bound for `K_8(8,4)`, verified via SAT-UNSAT + LRAT

`K_8(8,4) <= 505` is reformulated as: no assignment of a symbol to each of
the 8 coordinates makes the assignment simultaneously "far" (Hamming
distance > 4) from every one of the 505 codewords. This is exactly UNSAT of
...
The 505-word code is due to Mark Marosi, "New upper and lower bounds on
...
lrat_cnf_proof coverFormula coverProof
  (include_str "../../../../data/K_8_8_4/cover.cnf")
  (include_str "../../../../data/K_8_8_4/cover.lrat")
...
def oneHotClauses : Sat.Fmla :=
  (List.range 8).flatMap (fun j => atLeastClause j :: atMostClauses j)
...
/-- The 70 (= C(8,4)) "far from `c`" clauses: for every size-4 subset of
positions, at least one of them must disagree with `c`. -/
def farClauses (c : List Nat) : Sat.Fmla :=
  (combinations (List.range 8) 4).map (fun T =>
...
theorem coverFormula_eq_direct :
  ...
  have h1 : coverFormula.take 232 = oneHotClauses := by rfl
  have h2 : coverFormula.drop 232 = codeClauses code := by rfl
  have h := List.take_append_drop 232 coverFormula
  rw [h1, h2] at h
  exact h.symm

/-- The natural `Sat.Valuation` for a q-ary word: index `idx` is "on" iff
position `idx / 8` of `x` carries symbol `idx % 8`. -/
def qaryWordValuation (x : QaryWord 8 8) : Sat.Valuation :=
  fun idx => (x ⟨idx / 8 % 8, Nat.mod_lt _ (by decide)⟩).val = idx % 8

theorem qaryWordValuation_satVar (x : QaryWord 8 8) (j v : Nat) (hj : j < 8) (hv : v < 8) :
    qaryWordValuation x (satVar j v) ↔ (x ⟨j, hj⟩).val = v := by
  have hj' : (j * 8 + v) / 8 % 8 = j := by omega
  have hv' : (j * 8 + v) % 8 = v := by omega
  have hFin : (⟨(j * 8 + v) / 8 % 8, Nat.mod_lt _ (by decide)⟩ : Fin 8) = ⟨j, hj⟩ :=
    Fin.ext hj'
  show (x ⟨(j * 8 + v) / 8 % 8, Nat.mod_lt _ (by decide)⟩).val = (j * 8 + v) % 8 ↔
      (x ⟨j, hj⟩).val = v
  rw [hFin, hv']

theorem satisfies_atLeastClause (x : QaryWord 8 8) (j : Nat) (hj : j < 8) :
...
theorem satisfies_atMostPair (x : QaryWord 8 8) (j v1 v2 : Nat) (hj : j < 8) (hv1 : v1 < 8)
    (hv2 : v2 < 8) (hne : v1 ≠ v2) :
...
theorem satisfies_atMostClauses (x : QaryWord 8 8) (j : Nat) (hj : j < 8) (clause : Sat.Clause)
...
/-- Pigeonhole: if `f` and `g` disagree on at least 5 of the 8 positions,
...
theorem exists_disagree_of_far (f g : Nat → Nat)
    (hfar : 5 ≤ ((List.range 8).filter (fun j => decide (f j ≠ g j))).length) :
    ∀ T ∈ combinations (List.range 8) 4, ∃ j ∈ T, f j ≠ g j := by
  intro T hT
  by_contra hno
  push Not at hno
  obtain ⟨hsub, hlen⟩ := combinations_sublist (List.range 8) 4 T hT
  ...
  have hTfilter : T.filter (fun j => !p j) = T := by
    ...
    exact hno j hj
  have hTsub' : List.Sublist (T.filter (fun j => !p j)) ((List.range 8).filter (fun j => !p j)) :=
    List.Sublist.filter _ hsub
  rw [hTfilter] at hTsub'
  have hlen' : T.length ≤ ((List.range 8).filter (fun j => !p j)).length := hTsub'.length_le
  have hpart := length_filter_add_filter_not p (List.range 8)
  simp only [List.length_range] at hpart
  omega

/-- Total Nat-valued accessor for a `QaryWord`'s symbol at position `j`
(`0` out of range) -- avoids threading `j < 8` proofs through existentials. -/
def xVal (x : QaryWord 8 8) (j : Nat) : Nat :=
  if h : j < 8 then (x ⟨j, h⟩).val else 0
...
theorem satisfies_farClauseOne (x : QaryWord 8 8) (c : List Nat) (T : List Nat)
    (hTN : ∀ j ∈ T, j < 8) (hcQ : ∀ j, c.getD j 0 < 8)
...
theorem satisfies_farClauses (x : QaryWord 8 8) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 8)
    (hfar : 5 ≤ ((List.range 8).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length)
    (clause : Sat.Clause) (hmem : clause ∈ farClauses c) :
    (qaryWordValuation x).satisfies clause := by
  unfold farClauses at hmem
  obtain ⟨T, hT, rfl⟩ := List.mem_map.mp hmem
  have hTN : ∀ j ∈ T, j < 8 := fun j hj =>
...
theorem satisfies_codeClauses (x : QaryWord 8 8) (code : List (List Nat))
    (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 8)
    (hfar : ∀ c ∈ code, 5 ≤ ((List.range 8).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length)
...
/-- The actual covering theorem: every `QaryWord 8 8` is within Hamming
...
theorem covers (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 8) (x : QaryWord 8 8) :
    ∃ c ∈ code, ((List.range 8).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length ≤ 4 := by
  by_contra hno
  push Not at hno
...
/-- Convert a length-8, digits-`<8` `List Nat` codeword into an actual
`QaryWord 8 8`. The `% 8` is a safety net (irrelevant for well-formed
codewords, where `c.getD j 0 < 8` already). -/
def toQaryWord (c : List Nat) : QaryWord 8 8 :=
  fun i => ⟨c.getD i.val 0 % 8, Nat.mod_lt _ (by decide)⟩

theorem hammingDist_le_of_count_le (x : QaryWord 8 8) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 8)
    (n : Nat) (hcount : ((List.range 8).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length ≤ n) :
    hammingDist x (toQaryWord c) ≤ n := by
  have hcard : (Finset.univ.filter (fun i : Fin 8 => x i ≠ toQaryWord c i)).card ≤ n := by
    have hinj : Set.InjOn Fin.val (Finset.univ.filter (fun i : Fin 8 => x i ≠ toQaryWord c i) : Set (Fin 8)) :=
      fun a _ b _ h => Fin.ext h
    have hmaps : ∀ i ∈ Finset.univ.filter (fun i : Fin 8 => x i ≠ toQaryWord c i),
        i.val ∈ ((List.range 8).filter (fun j => decide (xVal x j ≠ c.getD j 0))).toFinset := by
      intro i hi
      ...
      simp only [hmod]
      rw [← hxi, heq]
    calc (Finset.univ.filter (fun i : Fin 8 => x i ≠ toQaryWord c i)).card
        ≤ ((List.range 8).filter (fun j => decide (xVal x j ≠ c.getD j 0))).toFinset.card :=
          Finset.card_le_card_of_injOn Fin.val hmaps hinj
      _ ≤ ((List.range 8).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length :=
...
def codeFinset : Finset (QaryWord 8 8) := (code.map toQaryWord).toFinset

set_option maxRecDepth 10000 in
theorem code_length : code.length = 505 := by decide

theorem codeFinset_card_le : codeFinset.card ≤ 505 := by
...
def explicitUpper : ExplicitQaryUpper 8 8 4 505 :=
  { code := codeFinset
...
def upper (q n r : Nat) : Nat :=
  if q = 8 ∧ n = 8 ∧ r = 4 then 505 else trivialUpper q n r

theorem upper_valid (q n r : Nat) :
    QaryKUpper q n r (upper q n r) := by
  by_cases h : q = 8 ∧ n = 8 ∧ r = 4
