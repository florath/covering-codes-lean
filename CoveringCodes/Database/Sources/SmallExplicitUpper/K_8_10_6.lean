...
/-!
# Explicit upper bound for `K_8(10,6)`, verified via SAT-UNSAT + LRAT

`K_8(10,6) <= 324` is reformulated as: no assignment of a symbol to each of
the 10 coordinates makes the assignment simultaneously "far" (Hamming
distance > 6) from every one of the 324 codewords. This is exactly UNSAT of
a CNF built from one-hot coordinate-symbol variables, plus, per codeword, a
"disagree in >= 7 positions" cardinality constraint (encoded directly as
clauses over the negated one-hot literals -- no auxiliary variables).

The 324-word code is due to Mark Marosi, "New upper and lower bounds on
...
lrat_cnf_proof coverFormula coverProof
  (include_str "../../../../data/K_8_10_6/cover.cnf")
  (include_str "../../../../data/K_8_10_6/cover.lrat")

open Mathlib.Tactic.Sat (buildConj buildClause)

/-- 0-indexed SAT variable for "position `j` has symbol `v`". -/
def satVar (j v : Nat) : Nat := j * 8 + v

def atLeastClause (j : Nat) : Sat.Clause :=
  (List.range 8).map (fun v => Sat.Literal.pos (satVar j v))

def atMostClauses (j : Nat) : Sat.Fmla :=
  (List.range 8).flatMap (fun v1 =>
    ((List.range 8).filter (v1 < ·)).map (fun v2 =>
      ([Sat.Literal.neg (satVar j v1), Sat.Literal.neg (satVar j v2)] : Sat.Clause)))

def oneHotClauses : Sat.Fmla :=
  (List.range 10).flatMap (fun j => atLeastClause j :: atMostClauses j)
...
/-- The 210 (= C(10,4)) "far from `c`" clauses: for every size-4 subset of
positions, at least one of them must disagree with `c`. -/
def farClauses (c : List Nat) : Sat.Fmla :=
  (combinations (List.range 10) 4).map (fun T =>
...
theorem coverFormula_eq_direct :
  ...
  have h1 : coverFormula.take 290 = oneHotClauses := by rfl
  have h2 : coverFormula.drop 290 = codeClauses code := by rfl
  have h := List.take_append_drop 290 coverFormula
  rw [h1, h2] at h
  exact h.symm

/-- The natural `Sat.Valuation` for a q-ary word: index `idx` is "on" iff
position `idx / 8` of `x` carries symbol `idx % 8`. -/
def qaryWordValuation (x : QaryWord 8 10) : Sat.Valuation :=
  fun idx => (x ⟨idx / 8 % 10, Nat.mod_lt _ (by decide)⟩).val = idx % 8

theorem qaryWordValuation_satVar (x : QaryWord 8 10) (j v : Nat) (hj : j < 10) (hv : v < 8) :
    qaryWordValuation x (satVar j v) ↔ (x ⟨j, hj⟩).val = v := by
  have hj' : (j * 8 + v) / 8 % 10 = j := by omega
  have hv' : (j * 8 + v) % 8 = v := by omega
  have hFin : (⟨(j * 8 + v) / 8 % 10, Nat.mod_lt _ (by decide)⟩ : Fin 10) = ⟨j, hj⟩ :=
    Fin.ext hj'
  show (x ⟨(j * 8 + v) / 8 % 10, Nat.mod_lt _ (by decide)⟩).val = (j * 8 + v) % 8 ↔
      (x ⟨j, hj⟩).val = v
  rw [hFin, hv']

theorem satisfies_atLeastClause (x : QaryWord 8 10) (j : Nat) (hj : j < 10) :
    (qaryWordValuation x).satisfies (atLeastClause j) := by
  have hlt : (x ⟨j, hj⟩).val < 8 := (x ⟨j, hj⟩).isLt
...
theorem satisfies_atMostPair (x : QaryWord 8 10) (j v1 v2 : Nat) (hj : j < 10) (hv1 : v1 < 8)
    (hv2 : v2 < 8) (hne : v1 ≠ v2) :
...
theorem satisfies_atMostClauses (x : QaryWord 8 10) (j : Nat) (hj : j < 10) (clause : Sat.Clause)
...
/-- Pigeonhole: if `f` and `g` disagree on at least 7 of the 10 positions,
...
theorem exists_disagree_of_far (f g : Nat → Nat)
    (hfar : 7 ≤ ((List.range 10).filter (fun j => decide (f j ≠ g j))).length) :
    ∀ T ∈ combinations (List.range 10) 4, ∃ j ∈ T, f j ≠ g j := by
  intro T hT
  by_contra hno
  push Not at hno
  obtain ⟨hsub, hlen⟩ := combinations_sublist (List.range 10) 4 T hT
  ...
  have hTfilter : T.filter (fun j => !p j) = T := by
    ...
    exact hno j hj
  have hTsub' : List.Sublist (T.filter (fun j => !p j)) ((List.range 10).filter (fun j => !p j)) :=
    List.Sublist.filter _ hsub
  rw [hTfilter] at hTsub'
  have hlen' : T.length ≤ ((List.range 10).filter (fun j => !p j)).length := hTsub'.length_le
  have hpart := length_filter_add_filter_not p (List.range 10)
  simp only [List.length_range] at hpart
  omega

/-- Total Nat-valued accessor for a `QaryWord`'s symbol at position `j`
(`0` out of range) -- avoids threading `j < 10` proofs through existentials. -/
def xVal (x : QaryWord 8 10) (j : Nat) : Nat :=
  if h : j < 10 then (x ⟨j, h⟩).val else 0
...
theorem satisfies_farClauseOne (x : QaryWord 8 10) (c : List Nat) (T : List Nat)
    (hTN : ∀ j ∈ T, j < 10) (hcQ : ∀ j, c.getD j 0 < 8)
...
theorem satisfies_farClauses (x : QaryWord 8 10) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 8)
    (hfar : 7 ≤ ((List.range 10).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length)
    (clause : Sat.Clause) (hmem : clause ∈ farClauses c) :
    (qaryWordValuation x).satisfies clause := by
  unfold farClauses at hmem
  obtain ⟨T, hT, rfl⟩ := List.mem_map.mp hmem
  have hTN : ∀ j ∈ T, j < 10 := fun j hj =>
...
theorem satisfies_codeClauses (x : QaryWord 8 10) (code : List (List Nat))
    (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 8)
    (hfar : ∀ c ∈ code, 7 ≤ ((List.range 10).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length)
...
/-- The actual covering theorem: every `QaryWord 8 10` is within Hamming
distance `6` of some codeword in `code` -- reached entirely via the
SAT-UNSAT/LRAT proof, no `decide`/`native_decide` anywhere in this chain. -/
theorem covers (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 8) (x : QaryWord 8 10) :
    ∃ c ∈ code, ((List.range 10).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length ≤ 6 := by
  by_contra hno
  push Not at hno
...
/-- Convert a length-10, digits-`<8` `List Nat` codeword into an actual
`QaryWord 8 10`. The `% 8` is a safety net (irrelevant for well-formed
codewords, where `c.getD j 0 < 8` already). -/
def toQaryWord (c : List Nat) : QaryWord 8 10 :=
  fun i => ⟨c.getD i.val 0 % 8, Nat.mod_lt _ (by decide)⟩

theorem hammingDist_le_of_count_le (x : QaryWord 8 10) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 8)
    (n : Nat) (hcount : ((List.range 10).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length ≤ n) :
    hammingDist x (toQaryWord c) ≤ n := by
  have hcard : (Finset.univ.filter (fun i : Fin 10 => x i ≠ toQaryWord c i)).card ≤ n := by
    have hinj : Set.InjOn Fin.val (Finset.univ.filter (fun i : Fin 10 => x i ≠ toQaryWord c i) : Set (Fin 10)) :=
      fun a _ b _ h => Fin.ext h
    have hmaps : ∀ i ∈ Finset.univ.filter (fun i : Fin 10 => x i ≠ toQaryWord c i),
        i.val ∈ ((List.range 10).filter (fun j => decide (xVal x j ≠ c.getD j 0))).toFinset := by
      intro i hi
      ...
      have hyi : (toQaryWord c) i = ⟨c.getD i.val 0 % 8, Nat.mod_lt _ (by decide)⟩ := rfl
      have hmod : c.getD i.val 0 % 8 = c.getD i.val 0 := Nat.mod_eq_of_lt (hcQ i.val)
      apply Fin.ext
      rw [hyi]
      simp only [hmod]
      rw [← hxi, heq]
    calc (Finset.univ.filter (fun i : Fin 10 => x i ≠ toQaryWord c i)).card
        ≤ ((List.range 10).filter (fun j => decide (xVal x j ≠ c.getD j 0))).toFinset.card :=
          Finset.card_le_card_of_injOn Fin.val hmaps hinj
      _ ≤ ((List.range 10).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length :=
          List.toFinset_card_le _
      _ ≤ n := hcount
  exact hcard

theorem getD_lt_of_forall_mem_lt {c : List Nat} (h : ∀ d ∈ c, d < 8) : ∀ j, c.getD j 0 < 8 := by
  induction c with
...
theorem code_entries_ltQ : ∀ c ∈ code, ∀ d ∈ c, d < 8 := by decide

theorem code_getD_lt : ∀ c ∈ code, ∀ j, c.getD j 0 < 8 :=
  fun c hc => getD_lt_of_forall_mem_lt (code_entries_ltQ c hc)

def codeFinset : Finset (QaryWord 8 10) := (code.map toQaryWord).toFinset

set_option maxRecDepth 10000 in
theorem code_length : code.length = 324 := by decide

theorem codeFinset_card_le : codeFinset.card ≤ 324 := by
  calc codeFinset.card
      ≤ (code.map toQaryWord).length := List.toFinset_card_le _
    _ = code.length := by rw [List.length_map]
    _ = 324 := code_length

theorem codeFinset_covers : CoversFinset codeFinset 6 := by
  intro x
  ...
  · exact hammingDist_le_of_count_le x c (code_getD_lt c hc) 6 hcount

def explicitUpper : ExplicitQaryUpper 8 10 6 324 :=
  { code := codeFinset
...
def upper (q n r : Nat) : Nat :=
  if q = 8 ∧ n = 10 ∧ r = 6 then 324 else trivialUpper q n r

theorem upper_valid (q n r : Nat) :
    QaryKUpper q n r (upper q n r) := by
  by_cases h : q = 8 ∧ n = 10 ∧ r = 6
