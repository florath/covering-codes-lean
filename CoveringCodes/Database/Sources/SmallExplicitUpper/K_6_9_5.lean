...
/-!
# Explicit upper bound for `K_6(9,5)`, verified via SAT-UNSAT + LRAT

`K_6(9,5) <= 119` is reformulated as: no assignment of a symbol to each of
the 9 coordinates makes the assignment simultaneously "far" (Hamming
distance > 5) from every one of the 119 codewords. This is exactly UNSAT of
...
The 119-word code is due to Mark Marosi, "New upper and lower bounds on
...
lrat_cnf_proof coverFormula coverProof
  (include_str "../../../../data/K_6_9_5/cover.cnf")
  (include_str "../../../../data/K_6_9_5/cover.lrat")
...
set_option maxRecDepth 10000 in
theorem code_length : code.length = 119 := by decide

theorem codeFinset_card_le : codeFinset.card ≤ 119 := by
...
def explicitUpper : ExplicitQaryUpper 6 9 5 119 :=
  { code := codeFinset
...
def upper (q n r : Nat) : Nat :=
  if q = 6 ∧ n = 9 ∧ r = 5 then 119 else trivialUpper q n r

theorem upper_valid (q n r : Nat) :
    QaryKUpper q n r (upper q n r) := by
  by_cases h : q = 6 ∧ n = 9 ∧ r = 5
