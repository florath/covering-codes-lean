...
/-!
# Explicit upper bound for `K_6(8,3)`, verified via SAT-UNSAT + LRAT

`K_6(8,3) <= 1005` is reformulated as: no assignment of a symbol to each of
the 8 coordinates makes the assignment simultaneously "far" (Hamming
distance > 3) from every one of the 1005 codewords. This is exactly UNSAT of
...
The 1005-word code is due to Mark Marosi, "New upper and lower bounds on
...
lrat_cnf_proof coverFormula coverProof
  (include_str "../../../../data/K_6_8_3/cover.cnf")
  (include_str "../../../../data/K_6_8_3/cover.lrat")
...
/-- The 56 (= C(8,5)) "far from `c`" clauses: for every size-5 subset of
positions, at least one of them must disagree with `c`. -/
def farClauses (c : List Nat) : Sat.Fmla :=
  (combinations (List.range 8) 5).map (fun T =>
...
/-- Pigeonhole: if `f` and `g` disagree on at least 4 of the 8 positions,
...
theorem exists_disagree_of_far (f g : Nat → Nat)
    (hfar : 4 ≤ ((List.range 8).filter (fun j => decide (f j ≠ g j))).length) :
    ∀ T ∈ combinations (List.range 8) 5, ∃ j ∈ T, f j ≠ g j := by
  intro T hT
  by_contra hno
  push Not at hno
  obtain ⟨hsub, hlen⟩ := combinations_sublist (List.range 8) 5 T hT
...
theorem satisfies_farClauses (x : QaryWord 6 8) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 6)
    (hfar : 4 ≤ ((List.range 8).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length)
...
theorem satisfies_codeClauses (x : QaryWord 6 8) (code : List (List Nat))
    ...
    (hfar : ∀ c ∈ code, 4 ≤ ((List.range 8).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length)
...
distance `3` of some codeword in `code` -- reached entirely via the
...
theorem covers (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 6) (x : QaryWord 6 8) :
    ∃ c ∈ code, ((List.range 8).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length ≤ 3 := by
  by_contra hno
  push Not at hno
...
set_option maxRecDepth 10000 in
theorem code_length : code.length = 1005 := by decide

theorem codeFinset_card_le : codeFinset.card ≤ 1005 := by
  calc codeFinset.card
      ≤ (code.map toQaryWord).length := List.toFinset_card_le _
    _ = code.length := by rw [List.length_map]
    _ = 1005 := code_length

theorem codeFinset_covers : CoversFinset codeFinset 3 := by
  intro x
  ...
  · exact hammingDist_le_of_count_le x c (code_getD_lt c hc) 3 hcount

def explicitUpper : ExplicitQaryUpper 6 8 3 1005 :=
  { code := codeFinset
...
def upper (q n r : Nat) : Nat :=
  if q = 6 ∧ n = 8 ∧ r = 3 then 1005 else trivialUpper q n r

theorem upper_valid (q n r : Nat) :
    QaryKUpper q n r (upper q n r) := by
  by_cases h : q = 6 ∧ n = 8 ∧ r = 3
