...
/-!
# Explicit upper bound for `K_6(9,4)`, verified via SAT-UNSAT + LRAT

`K_6(9,4) <= 660` is reformulated as: no assignment of a symbol to each of
the 9 coordinates makes the assignment simultaneously "far" (Hamming
distance > 4) from every one of the 660 codewords. This is exactly UNSAT of
...
The 660-word code is due to Mark Marosi, "New upper and lower bounds on
...
lrat_cnf_proof coverFormula coverProof
  (include_str "../../../../data/K_6_9_4/cover.cnf")
  (include_str "../../../../data/K_6_9_4/cover.lrat")
...
/-- The 126 (= C(9,5)) "far from `c`" clauses: for every size-5 subset of
positions, at least one of them must disagree with `c`. -/
def farClauses (c : List Nat) : Sat.Fmla :=
  (combinations (List.range 9) 5).map (fun T =>
...
theorem coverFormula_eq_direct :
  ...
  have h1 : coverFormula.take 144 = oneHotClauses := by rfl
  have h2 : coverFormula.drop 144 = codeClauses code := by rfl
  have h := List.take_append_drop 144 coverFormula
  rw [h1, h2] at h
  exact h.symm

/-- The natural `Sat.Valuation` for a q-ary word: index `idx` is "on" iff
position `idx / 6` of `x` carries symbol `idx % 6`. -/
def qaryWordValuation (x : QaryWord 6 9) : Sat.Valuation :=
  fun idx => (x ⟨idx / 6 % 9, Nat.mod_lt _ (by decide)⟩).val = idx % 6

theorem qaryWordValuation_satVar (x : QaryWord 6 9) (j v : Nat) (hj : j < 9) (hv : v < 6) :
    qaryWordValuation x (satVar j v) ↔ (x ⟨j, hj⟩).val = v := by
  have hj' : (j * 6 + v) / 6 % 9 = j := by omega
  have hv' : (j * 6 + v) % 6 = v := by omega
  have hFin : (⟨(j * 6 + v) / 6 % 9, Nat.mod_lt _ (by decide)⟩ : Fin 9) = ⟨j, hj⟩ :=
    Fin.ext hj'
  show (x ⟨(j * 6 + v) / 6 % 9, Nat.mod_lt _ (by decide)⟩).val = (j * 6 + v) % 6 ↔
      (x ⟨j, hj⟩).val = v
  rw [hFin, hv']

theorem satisfies_atLeastClause (x : QaryWord 6 9) (j : Nat) (hj : j < 9) :
...
theorem satisfies_atMostPair (x : QaryWord 6 9) (j v1 v2 : Nat) (hj : j < 9) (hv1 : v1 < 6)
    (hv2 : v2 < 6) (hne : v1 ≠ v2) :
...
theorem satisfies_atMostClauses (x : QaryWord 6 9) (j : Nat) (hj : j < 9) (clause : Sat.Clause)
...
theorem satisfies_oneHotClauses (x : QaryWord 6 9) (clause : Sat.Clause)
...
/-- Pigeonhole: if `f` and `g` disagree on at least 5 of the 9 positions,
every size-5 subset of positions contains a disagreement. -/
...
theorem exists_disagree_of_far (f g : Nat → Nat)
    (hfar : 5 ≤ ((List.range 9).filter (fun j => decide (f j ≠ g j))).length) :
    ∀ T ∈ combinations (List.range 9) 5, ∃ j ∈ T, f j ≠ g j := by
  intro T hT
  by_contra hno
  push Not at hno
  obtain ⟨hsub, hlen⟩ := combinations_sublist (List.range 9) 5 T hT
...
def xVal (x : QaryWord 6 9) (j : Nat) : Nat :=
...
theorem satisfies_farClauseOne (x : QaryWord 6 9) (c : List Nat) (T : List Nat)
    (hTN : ∀ j ∈ T, j < 9) (hcQ : ∀ j, c.getD j 0 < 6)
...
theorem satisfies_farClauses (x : QaryWord 6 9) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 6)
    (hfar : 5 ≤ ((List.range 9).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length)
...
theorem satisfies_codeClauses (x : QaryWord 6 9) (code : List (List Nat))
    (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 6)
    (hfar : ∀ c ∈ code, 5 ≤ ((List.range 9).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length)
...
/-- The actual covering theorem: every `QaryWord 6 9` is within Hamming
...
theorem covers (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 6) (x : QaryWord 6 9) :
    ∃ c ∈ code, ((List.range 9).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length ≤ 4 := by
  by_contra hno
  push Not at hno
...
/-- Convert a length-9, digits-`<6` `List Nat` codeword into an actual
`QaryWord 6 9`. The `% 6` is a safety net (irrelevant for well-formed
codewords, where `c.getD j 0 < 6` already). -/
def toQaryWord (c : List Nat) : QaryWord 6 9 :=
  fun i => ⟨c.getD i.val 0 % 6, Nat.mod_lt _ (by decide)⟩

theorem hammingDist_le_of_count_le (x : QaryWord 6 9) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 6)
...
def codeFinset : Finset (QaryWord 6 9) := (code.map toQaryWord).toFinset

set_option maxRecDepth 10000 in
theorem code_length : code.length = 660 := by decide

theorem codeFinset_card_le : codeFinset.card ≤ 660 := by
...
def explicitUpper : ExplicitQaryUpper 6 9 4 660 :=
  { code := codeFinset
...
def upper (q n r : Nat) : Nat :=
  if q = 6 ∧ n = 9 ∧ r = 4 then 660 else trivialUpper q n r

theorem upper_valid (q n r : Nat) :
    QaryKUpper q n r (upper q n r) := by
  by_cases h : q = 6 ∧ n = 9 ∧ r = 4
