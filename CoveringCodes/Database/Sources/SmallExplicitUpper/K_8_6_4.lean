...
/-!
# Explicit upper bound for `K_8(6,4)`, verified via SAT-UNSAT + LRAT

`K_8(6,4) <= 19` is reformulated as: no assignment of a symbol to each of
the 6 coordinates makes the assignment simultaneously "far" (Hamming
distance > 4) from every one of the 19 codewords. This is exactly UNSAT of
...
The 19-word code is due to Mark Marosi, "New upper and lower bounds on
...
lrat_cnf_proof coverFormula coverProof
  (include_str "../../../../data/K_8_6_4/cover.cnf")
  (include_str "../../../../data/K_8_6_4/cover.lrat")
...
theorem coverFormula_eq_direct :
  ...
  have h1 : coverFormula.take 174 = oneHotClauses := by rfl
  have h2 : coverFormula.drop 174 = codeClauses code := by rfl
  have h := List.take_append_drop 174 coverFormula
  rw [h1, h2] at h
  exact h.symm

/-- The natural `Sat.Valuation` for a q-ary word: index `idx` is "on" iff
position `idx / 8` of `x` carries symbol `idx % 8`. -/
def qaryWordValuation (x : QaryWord 8 6) : Sat.Valuation :=
  fun idx => (x ⟨idx / 8 % 6, Nat.mod_lt _ (by decide)⟩).val = idx % 8

theorem qaryWordValuation_satVar (x : QaryWord 8 6) (j v : Nat) (hj : j < 6) (hv : v < 8) :
    qaryWordValuation x (satVar j v) ↔ (x ⟨j, hj⟩).val = v := by
  have hj' : (j * 8 + v) / 8 % 6 = j := by omega
  have hv' : (j * 8 + v) % 8 = v := by omega
  have hFin : (⟨(j * 8 + v) / 8 % 6, Nat.mod_lt _ (by decide)⟩ : Fin 6) = ⟨j, hj⟩ :=
    Fin.ext hj'
  show (x ⟨(j * 8 + v) / 8 % 6, Nat.mod_lt _ (by decide)⟩).val = (j * 8 + v) % 8 ↔
      (x ⟨j, hj⟩).val = v
  rw [hFin, hv']

theorem satisfies_atLeastClause (x : QaryWord 8 6) (j : Nat) (hj : j < 6) :
...
theorem satisfies_atMostPair (x : QaryWord 8 6) (j v1 v2 : Nat) (hj : j < 6) (hv1 : v1 < 8)
    (hv2 : v2 < 8) (hne : v1 ≠ v2) :
...
theorem satisfies_atMostClauses (x : QaryWord 8 6) (j : Nat) (hj : j < 6) (clause : Sat.Clause)
...
theorem satisfies_oneHotClauses (x : QaryWord 8 6) (clause : Sat.Clause)
...
def xVal (x : QaryWord 8 6) (j : Nat) : Nat :=
...
theorem satisfies_farClauseOne (x : QaryWord 8 6) (c : List Nat) (T : List Nat)
    (hTN : ∀ j ∈ T, j < 6) (hcQ : ∀ j, c.getD j 0 < 8)
...
theorem satisfies_farClauses (x : QaryWord 8 6) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 8)
...
theorem satisfies_codeClauses (x : QaryWord 8 6) (code : List (List Nat))
...
/-- The actual covering theorem: every `QaryWord 8 6` is within Hamming
...
theorem covers (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 8) (x : QaryWord 8 6) :
...
/-- Convert a length-6, digits-`<8` `List Nat` codeword into an actual
`QaryWord 8 6`. The `% 8` is a safety net (irrelevant for well-formed
codewords, where `c.getD j 0 < 8` already). -/
def toQaryWord (c : List Nat) : QaryWord 8 6 :=
  fun i => ⟨c.getD i.val 0 % 8, Nat.mod_lt _ (by decide)⟩

theorem hammingDist_le_of_count_le (x : QaryWord 8 6) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 8)
...
def codeFinset : Finset (QaryWord 8 6) := (code.map toQaryWord).toFinset

set_option maxRecDepth 10000 in
theorem code_length : code.length = 19 := by decide

theorem codeFinset_card_le : codeFinset.card ≤ 19 := by
...
def explicitUpper : ExplicitQaryUpper 8 6 4 19 :=
  { code := codeFinset
...
def upper (q n r : Nat) : Nat :=
  if q = 8 ∧ n = 6 ∧ r = 4 then 19 else trivialUpper q n r

theorem upper_valid (q n r : Nat) :
    QaryKUpper q n r (upper q n r) := by
  by_cases h : q = 8 ∧ n = 6 ∧ r = 4
