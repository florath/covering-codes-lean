...
/-!
# Explicit upper bound for `K_6(8,4)`, verified via SAT-UNSAT + LRAT

`K_6(8,4) <= 166` is reformulated as: no assignment of a symbol to each of
the 8 coordinates makes the assignment simultaneously "far" (Hamming
distance > 4) from every one of the 166 codewords. This is exactly UNSAT of
...
The 166-word code is due to Mark Marosi, "New upper and lower bounds on
...
lrat_cnf_proof coverFormula coverProof
  (include_str "../../../../data/K_6_8_4/cover.cnf")
  (include_str "../../../../data/K_6_8_4/cover.lrat")
...
theorem coverFormula_eq_direct :
  ...
  have h1 : coverFormula.take 128 = oneHotClauses := by rfl
  have h2 : coverFormula.drop 128 = codeClauses code := by rfl
  have h := List.take_append_drop 128 coverFormula
  rw [h1, h2] at h
  exact h.symm

/-- The natural `Sat.Valuation` for a q-ary word: index `idx` is "on" iff
position `idx / 6` of `x` carries symbol `idx % 6`. -/
def qaryWordValuation (x : QaryWord 6 8) : Sat.Valuation :=
  fun idx => (x ⟨idx / 6 % 8, Nat.mod_lt _ (by decide)⟩).val = idx % 6

theorem qaryWordValuation_satVar (x : QaryWord 6 8) (j v : Nat) (hj : j < 8) (hv : v < 6) :
    qaryWordValuation x (satVar j v) ↔ (x ⟨j, hj⟩).val = v := by
  have hj' : (j * 6 + v) / 6 % 8 = j := by omega
  have hv' : (j * 6 + v) % 6 = v := by omega
  have hFin : (⟨(j * 6 + v) / 6 % 8, Nat.mod_lt _ (by decide)⟩ : Fin 8) = ⟨j, hj⟩ :=
    Fin.ext hj'
  show (x ⟨(j * 6 + v) / 6 % 8, Nat.mod_lt _ (by decide)⟩).val = (j * 6 + v) % 6 ↔
      (x ⟨j, hj⟩).val = v
  rw [hFin, hv']

theorem satisfies_atLeastClause (x : QaryWord 6 8) (j : Nat) (hj : j < 8) :
...
theorem satisfies_atMostPair (x : QaryWord 6 8) (j v1 v2 : Nat) (hj : j < 8) (hv1 : v1 < 6)
    (hv2 : v2 < 6) (hne : v1 ≠ v2) :
...
theorem satisfies_atMostClauses (x : QaryWord 6 8) (j : Nat) (hj : j < 8) (clause : Sat.Clause)
...
theorem satisfies_oneHotClauses (x : QaryWord 6 8) (clause : Sat.Clause)
...
def xVal (x : QaryWord 6 8) (j : Nat) : Nat :=
...
theorem satisfies_farClauseOne (x : QaryWord 6 8) (c : List Nat) (T : List Nat)
    (hTN : ∀ j ∈ T, j < 8) (hcQ : ∀ j, c.getD j 0 < 6)
...
theorem satisfies_farClauses (x : QaryWord 6 8) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 6)
...
theorem satisfies_codeClauses (x : QaryWord 6 8) (code : List (List Nat))
    (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 6)
...
/-- The actual covering theorem: every `QaryWord 6 8` is within Hamming
...
theorem covers (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 6) (x : QaryWord 6 8) :
...
/-- Convert a length-8, digits-`<6` `List Nat` codeword into an actual
`QaryWord 6 8`. The `% 6` is a safety net (irrelevant for well-formed
codewords, where `c.getD j 0 < 6` already). -/
def toQaryWord (c : List Nat) : QaryWord 6 8 :=
  fun i => ⟨c.getD i.val 0 % 6, Nat.mod_lt _ (by decide)⟩

theorem hammingDist_le_of_count_le (x : QaryWord 6 8) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 6)
  ...
  have hcard : (Finset.univ.filter (fun i : Fin 8 => x i ≠ toQaryWord c i)).card ≤ n := by
    ...
    have hmaps : ∀ i ∈ Finset.univ.filter (fun i : Fin 8 => x i ≠ toQaryWord c i),
      ...
      have hmod : c.getD i.val 0 % 6 = c.getD i.val 0 := Nat.mod_eq_of_lt (hcQ i.val)
      apply Fin.ext
      rw [hyi]
      simp only [hmod]
      rw [← hxi, heq]
...
theorem code_entries_ltQ : ∀ c ∈ code, ∀ d ∈ c, d < 6 := by decide

theorem code_getD_lt : ∀ c ∈ code, ∀ j, c.getD j 0 < 6 :=
  fun c hc => getD_lt_of_forall_mem_lt (code_entries_ltQ c hc)

def codeFinset : Finset (QaryWord 6 8) := (code.map toQaryWord).toFinset

set_option maxRecDepth 10000 in
theorem code_length : code.length = 166 := by decide

theorem codeFinset_card_le : codeFinset.card ≤ 166 := by
...
def explicitUpper : ExplicitQaryUpper 6 8 4 166 :=
  { code := codeFinset
...
def upper (q n r : Nat) : Nat :=
  if q = 6 ∧ n = 8 ∧ r = 4 then 166 else trivialUpper q n r

theorem upper_valid (q n r : Nat) :
    QaryKUpper q n r (upper q n r) := by
  by_cases h : q = 6 ∧ n = 8 ∧ r = 4
