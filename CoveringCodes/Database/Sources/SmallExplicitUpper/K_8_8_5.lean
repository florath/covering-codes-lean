...
/-!
# Explicit upper bound for `K_8(8,5)`, verified via SAT-UNSAT + LRAT

`K_8(8,5) <= 79` is reformulated as: no assignment of a symbol to each of
the 8 coordinates makes the assignment simultaneously "far" (Hamming
distance > 5) from every one of the 79 codewords. This is exactly UNSAT of
...
The 79-word code is due to Mark Marosi, "New upper and lower bounds on
...
lrat_cnf_proof coverFormula coverProof
  (include_str "../../../../data/K_8_8_5/cover.cnf")
  (include_str "../../../../data/K_8_8_5/cover.lrat")
...
/-- The 56 (= C(8,3)) "far from `c`" clauses: for every size-3 subset of
positions, at least one of them must disagree with `c`. -/
def farClauses (c : List Nat) : Sat.Fmla :=
  (combinations (List.range 8) 3).map (fun T =>
...
/-- Pigeonhole: if `f` and `g` disagree on at least 6 of the 8 positions,
every size-3 subset of positions contains a disagreement. -/
...
theorem exists_disagree_of_far (f g : Nat → Nat)
    (hfar : 6 ≤ ((List.range 8).filter (fun j => decide (f j ≠ g j))).length) :
    ∀ T ∈ combinations (List.range 8) 3, ∃ j ∈ T, f j ≠ g j := by
  intro T hT
  by_contra hno
  push Not at hno
  obtain ⟨hsub, hlen⟩ := combinations_sublist (List.range 8) 3 T hT
...
theorem satisfies_farClauses (x : QaryWord 8 8) (c : List Nat) (hcQ : ∀ j, c.getD j 0 < 8)
    (hfar : 6 ≤ ((List.range 8).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length)
...
theorem satisfies_codeClauses (x : QaryWord 8 8) (code : List (List Nat))
    ...
    (hfar : ∀ c ∈ code, 6 ≤ ((List.range 8).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length)
...
theorem covers (hcQ : ∀ c ∈ code, ∀ j, c.getD j 0 < 8) (x : QaryWord 8 8) :
    ∃ c ∈ code, ((List.range 8).filter (fun j => decide (xVal x j ≠ c.getD j 0))).length ≤ 5 := by
  by_contra hno
  push Not at hno
...
set_option maxRecDepth 10000 in
theorem code_length : code.length = 79 := by decide

theorem codeFinset_card_le : codeFinset.card ≤ 79 := by
...
def explicitUpper : ExplicitQaryUpper 8 8 5 79 :=
  { code := codeFinset
...
def upper (q n r : Nat) : Nat :=
  if q = 8 ∧ n = 8 ∧ r = 5 then 79 else trivialUpper q n r

theorem upper_valid (q n r : Nat) :
    QaryKUpper q n r (upper q n r) := by
  by_cases h : q = 8 ∧ n = 8 ∧ r = 5
